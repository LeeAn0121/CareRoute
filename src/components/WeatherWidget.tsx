  ...
  useEffect(() => {
    ...
    async function fetchWeather() {
      try {
        ...
        // 1. Get Weather from Open-Meteo (No CORS, No API Key, Fast)
          ...
          window.naver.maps.Service.reverseGeocode({
            ...
            if (status === window.naver.maps.Service.Status.OK && isMounted) {
              ...
              if (result && result.results && result.results.length > 0) {
                ...
                // Prefer Dong (e.g. 역삼동), fallback to Gu (e.g. 강남구)
    ...
    // Add a small debounce to prevent spamming APIs while dragging map quickly
